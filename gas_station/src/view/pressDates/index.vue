...
<script>
  import { tankDataQuery } from '../../api/api'
  export default {
    data () {
      return {
        tankerList: [],
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if (params.row.type === 1) {
                ...
                type = params.row.oilTankNumber + '#油罐'
              }
              return h('div', type)
            }
          },
    ...
    methods: {
      export2Excel () {
        for (let i = 0;i < this.dataTable.length; i++) {
          ...
          let type
          if (this.dataTable[i].type === 1) {
            type = '管道'
          } else {
            type = this.dataTable[i].oilTankNumber + '#油罐'
          }
          this.dataTable[i].name = type
        }
        require.ensure([], () => {
          const { export_json_to_excel } = require('@/vendor/Export2Excel')
          const tHeader = ['序号', '类型', '系统压力(pa)', '时间']
          const filterVal = ['index', 'name', 'value', 'time']
          const list = this.dataTable
          const data = this.formatJson(filterVal, list)
          let date = '压力数据' + this.formatDate(new Date())
