      ...
      <div class="echarts-content" style="margin-bottom: 20px;">
        <div class="card-top-box">
          ...
          <!--<div style="float: right">-->
            <!--<div v-for="(item, index) in tankList" :key="item.id" :class="checkedTank === index ? 'card-top-tank-checked' : 'card-top-tank-defult'"  @click="changeTank1(index)">{{item.name}}号罐</div>-->
    ...
    <Row :gutter="16">
      ...
      <div class="echarts-content" style="margin-bottom: 20px;">
        ...
        <div class="card-content">
          ...
          <div id="gas_oil_chart_ppm" style="width: 100%;height: 100%"></div>
        </div>
      </div>
      </Col>
      <Col span="12" v-show="showLEL">
      <div class="echarts-content" style="margin-bottom: 20px;">
      <div class="card-top-box">
      <div class="card-title-text">
      油气浓度数据分析
      ...
      <div id="gas_oil_chart_lel" style="width: 100%;height: 100%"></div>
      </div>
      </div>
      </Col>
    </Row>
...
<script>
  import { oliTankMonitor, tankAnalyse, tankLeakageAnalyse, gasOilAnalyse, environmentTemperatureAnalyse, pipelinePressureAnalyse } from '../../api/api'
  ...
  export default {
    ...
    mounted () {
      ...
      vm.$nextTick(function () {
        ...
        vm.paramnter2.stationId = JSON.parse(sessionStorage.getItem('station')).stationId
        ...
        vm.paramnter4.stationId = JSON.parse(sessionStorage.getItem('station')).stationId
    ...
    methods: {
      ...
      // },
      getTank () {
        pipelinePressureAnalyse(this.paramnter1).then((res) => {
          let edate = []
          if (res.data.code === 1000) {
      ...
      getGasOilPPM () {
        gasOilAnalyse(this.paramnter3).then((res) => {
          ...
          // 油气浓度有ppm单位的传感器
          if (res.data.code === 1000) {
            if (res.data.content.ppm !== undefined) {
              // 折线图数据
              for (let i = 0; i < res.data.content.ppm.length; i++) {
                let s = {
                  name: res.data.content.ppm[i].time,
                  value:[res.data.content.ppm[i].time, res.data.content.ppm[i].value]
                }
                edate.push(s)
              }
              this.showPPM = true
              this.drawLineGasOilPPM('gas_oil_chart_ppm', edate)
            }
          } else {
            // ppm单位的传感器不存在
            this.showPPM = false
      ...
      getGasOilLEL () {
        let vm = this
        vm.paramnter2.unitType = 2
        gasOilAnalyse(vm.paramnter2).then((res) => {
          let edate = []
          // 油气浓度有LEL单位的传感器
          if (res.data.code === 1000) {
            if (res.data.content.lel !== undefined) {
              // 折线图数据
              for (let i = 0; i < res.data.content.lel.length; i++) {
                let s = {
                  name: res.data.content.lel[i].time,
                  value:[res.data.content.lel[i].time, res.data.content.lel[i].value]
                }
                edate.push(s)
              }
              vm.showLEL = true
              vm.drawLineGasOilLEL('gas_oil_chart_lel', edate)
            }
          } else {
            // LEL单位的传感器不存在
            vm.showLEL = false
      ...
      getTemperature () {
        environmentTemperatureAnalyse(this.paramnter4).then((res) => {
          let edate = []
          if (res.data.code === 1000) {
            if (res.data.content.length > 0) {
              // 折线图数据
